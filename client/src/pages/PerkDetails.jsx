...
async function handleDelete() {
  if (window.confirm(`Are you sure you want to delete "${perk.title}"?`)) {
    try {
      // Assuming 'id' is defined in your component's scope (or use perk._id)
      await api.delete('/perks/' + id); 
      nav('/perks');
    } catch (err) {
      console.log(err); // Good practice to log the full error for debugging
